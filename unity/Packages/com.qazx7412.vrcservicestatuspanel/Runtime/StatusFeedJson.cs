...
namespace VRCServiceStatusPanel
{
    ...
    public class StatusFeedJson : UdonSharpBehaviour
    {
        ...
        /// <summary>
        /// doubleが隣り合う整数を区別できる限界（2^53）。この値自身は含めない。
        ///
        /// 2^53を超えると、整数はもう一つずつ表せない。2^53+1は2^53へ丸められるので、
        /// 2^53を受け取ってしまうと、入力が2^53だったのか2^53+1だったのか分からない。
        /// 未満で切れば、通る値はすべて入力そのものである。
        ///
        /// 大きすぎる値をここでまとめて落とす狙いもある。9223372036854775809は-2^63へ、
        /// 9223372036854775808は2^63へ丸められ、元の値には戻せない。
        /// 境界をlong側で刻んでも、丸めた結果を見ているだけになる。
        ///
