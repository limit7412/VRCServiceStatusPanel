...
続ける版は、zip の有無にかかわらず、先端で決め直して同じ版が出ることを確かめる。
出なければ公開済みの最新より古い版で、リリースが無ければ作りかけのタグとして消して先端から決め直し、リリースがあれば止まる。
消しきれずに zip 付きで残ったリリースを完了と見なすと、正式版より古いプレリリースがそのまま残る。
再実行までに `master` が進み、別の実行が次の版を出していても、そのタグに zip が無ければ、その内容で zip を作って付ける。
タグを作る前に落ちていれば、再実行は先端から版を決め直す。

次の版の計算は、そのとき取り直した `master` の先端から辿れて、zip の付いたリリースのあるタグだけを見る。
checkout した後に `master` が進み、その先で正式版が公開されることがあるので、手元の作業ツリーではなく取り直した先端を基準にする。
別のブランチや `master` の過去のコミットに打って公開を拒まれたタグが残っても、採番はそれで飛ばない。
逆に、zip の付いたリリースのあるタグが先端から辿れないときは止まる。
force push で履歴が外れると、公開済みの正式版より低い版まで戻るためである。
...
同じタグで実行が複数あるときは、そのすべてを見る。
検査を通った後で落ちたリリースがあると、直すか消すまで採番は止まる。
検査まで進まずに落ちたものも、拒まれたか分からないので止まる。
タグの形とリリースの種別（`X.Y.Z` は正式版、`X.Y.Z-testN` はプレリリース）が食い違っていても止まる。
新しく決めた版は、zip を作った後にもう一度、公開済みのタグと `master` の先端から決め直す。
どちらかが変わっていれば、出す先も版も揃え直して zip を作り直し、変わらなくなってから出す先を記録して、タグを打ち、リリースを作って zip を付ける。
記録を先に置くのは、公開の途中で落ちたときに、再実行がそのタグを見つけられるようにするためである。
公開した後にもう一度決め直し、同じ版が出れば確定する。
違えば、そのあいだに正式版が公開されている。作ったリリースとタグを消して止め、次の取り込みか再実行が作り直す。
...
プレリリースでなく公開されているリリースがあるときも同じに止まる。
`X.Y.Z-testN` を正式版として公開してしまった状態で zip を付けると、test 版が正式版として配られる。
...
`prod` へ出すのはそのタグの内容で、`master` の先端ではない。
zip を作っているあいだに `master` が進んでも、出るのは公開したタグのままである。
...
Actions が止まっていたあいだに公開された正式版のように、リリースはあるのに zip が無いものは、`release.yml` を手で流して直す。
`version` にそのタグを入れ、`attach` を立てると、そのリリースへ zip を添付する。
`attach` を立てなければ、手で流したときは artifact に置くだけで、リリースには触れない。

`prod` へ出すのは zip の添付が通ってからで、`release.yml` が `deploy.yml` を `master` の ref で `workflow_dispatch` として起こし、終わるまで待つ。
出す内容は正式版のタグで、`ref` の入力に渡す。
zip を作っているあいだに `master` が進んでも、出るのは公開したタグのままである。
