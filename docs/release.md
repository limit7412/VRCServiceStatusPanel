...
実行は決めた版とコミットを自分の artifact に記録し、再実行はそれを読む。
再実行までに `master` が進み、別の実行が次の版を出していても、記録した版のタグに zip が無ければ、そのタグの内容で zip を作って付ける。
...
正式版の zip は `release.yml` が公開の後で付けるので、zip の無い正式版があれば、その `release.yml` の build が終わるまで待ってから数える。
待たずに数えると、その正式版より古い `X.Y.Z-testN` が正式版の後に出る。
