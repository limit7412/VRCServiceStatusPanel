...
実行は出すコミットを自分の artifact に記録し、再実行はそれを読んで、そのコミットに打たれたタグを続ける。
...
新しく決めた版は、zip を作った後にもう一度決め直し、変わっていれば zip を作り直してから、同じステップでタグを打って push する。
打った後にもう一度決め直し、同じ版が出れば確定し、違えば打ったタグを消してやり直す。
決めてから打つまでのあいだに正式版が出ると、決めた版はその正式版より古くなるので、その窓を一つのステップの中に閉じている。
